...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(self, vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask):
        vid_fea = self.video_trans(vid_fea)
        if self.search_beam_size > 1:
            return self.beam_search(vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask)
    ...
    def beam_search(self, vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
