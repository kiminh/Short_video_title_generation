...
def truncate_tokens_pair(tokens_a, tokens_b, max_len, vid_len):
    if len(tokens_a) + len(tokens_b) + vid_len > max_len-3:
        while len(tokens_a) + len(tokens_b) + vid_len > max_len-3:
...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file, vid_file, vid_path, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[]):
        super().__init__()
        ...
        if 'train' in file:
            with open(vid_file, 'r') as ff:
                vid_lists = [line.split('\n')[0] for line in ff]
            self.vid_lists = [vid_path + ind for ind in vid_lists]
        #else:
         #   with open(vid_file, 'r') as ff:
          #      vid_lists = [line.split('\n')[0] for line in ff]
           # self.vid_lists = ['data/train_0.2_refined/' + ind for ind in vid_lists]

        # if 'test' in file:
        #     with open('data/vid_test.txt', 'r') as ff:
        #         vid_lists = [line for line in ff]
        #     self.vid_lists = ['data/test/' + ind for ind in vid_lists]
    ...
    def __getitem__(self, idx):
        ...
        vid_features = np.load(self.vid_lists[idx])
        # proc = choice(self.bi_uni_pipeline)
        new_instance = ()
        for proc in self.bi_uni_pipeline:
            new_instance += proc(instance, vid_features)
        return new_instance
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance, vid_fea):
        next_sentence_label = None
        tokens_a, tokens_b = instance[:2]
        tokens_a = self.tokenizer.tokenize(tokens_a)
        tokens_b = self.tokenizer.tokenize(tokens_b)
        vid_len = vid_fea.shape[0]
        # -3  for special tokens [CLS], [SEP], [SEP]
        tokens_a, tokens_b = truncate_tokens_pair(tokens_a, tokens_b, self.max_len, vid_len)
        # Add Special Tokens
        tokens = ['[CLS]'] + tokens_a + vid_len*['[unused1]'] + ['[SEP]'] + tokens_b + ['[SEP]']
        vid_start = len(tokens_a)+1
        segment_ids = [4]*(len(tokens_a)+2+vid_len) + [5]*(len(tokens_b)+1)
        ...
        for i, tk in enumerate(tokens):
            ...
            if (i >= len(tokens_a)+2+vid_len) and (tk != '[CLS]'):
                cand_pos.append(i)
        ...
        input_mask[:, :len(tokens_a)+2+vid_len].fill_(1)
        second_st, second_end = len(
            tokens_a)+2+vid_len, len(tokens_a)+len(tokens_b)+3+vid_len
        ...
        return (input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, next_sentence_label, vid_fea, vid_start)
...
class Preprocess4Seq2seqDecode(Pipeline):
    ...
    def __call__(self, instance, vid):
        tokens_a, max_a_len = instance
        vid_len = vid.shape[0]
        # Add Special Tokens
        padded_tokens_a = ['[CLS]'] + tokens_a + vid_len*['[unused1]'] + ['[SEP]']
        vid_start = len(tokens_a) + 1
        assert len(padded_tokens_a) <= max_a_len + 2 + vid_len
        if max_a_len + 2 + vid_len > len(padded_tokens_a):
            padded_tokens_a += ['[PAD]'] * \
                (max_a_len + 2 + vid_len - len(padded_tokens_a))
        assert len(padded_tokens_a) == max_a_len + 2 + vid_len
        max_len_in_batch = min(self.max_tgt_length +
                               max_a_len + 2 + vid_len, self.max_len)
        tokens = padded_tokens_a
        segment_ids = [4]*(len(padded_tokens_a)) + [5]*(max_len_in_batch - len(padded_tokens_a))

        position_ids = []
        for i in range(len(tokens_a) + 2 + vid_len):
            position_ids.append(i)
        for i in range(len(tokens_a) + 2 + vid_len, max_a_len + 2 + vid_len):
            position_ids.append(0)
        for i in range(max_a_len + 2 + vid_len, max_len_in_batch):
            position_ids.append(i - (max_a_len + 2 + vid_len) + len(tokens_a) + 2 + vid_len)
        ...
        return (input_ids, segment_ids, position_ids, input_mask, vid, vid_start)
