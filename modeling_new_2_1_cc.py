...
from modeling_bert import load_tf_weights_in_bert, BertPooler, BertIntermediate, BertOutput, BertPredictionHeadTransform, BertSelfOutput, BertLMPredictionHead, BertOnlyMLMHead, BertOnlyMLMHead, BertEmbeddings, BertOnlyNSPHead
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, output_attentions=False):
        self_output = self.self(
            input_tensor, attention_mask, history_states=history_states, output_attentions=output_attentions)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, output_attentions=False):
        attention_output = self.attention(
            hidden_states, attention_mask, history_states=history_states, output_attentions=output_attentions)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, output_attentions=False):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                ...
                att_pros = hidden_states[1:]
                all_att_pros.append(att_pros)
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, all_att_pros
...
class UnilmModel(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, vid_fea=None, vid_start=None, output_all_encoded_layers=True):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask) #eqution 2

        embedding_output = self.embeddings(
            input_ids, token_type_ids, vid_fea=vid_fea, vid_start=vid_start)
        encoded_layers = self.encoder(embedding_output, extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers) # encoder output
        sequence_output = encoded_layers[-1]# the last hidden
...
class UnilmModelIncr(UnilmModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, vid_fea, vid_start, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, output_attentions=False):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)

        embedding_output = self.embeddings(
            input_ids, token_type_ids, position_ids, vid_fea=vid_fea, vid_start=vid_start)
        encoded_layers, all_att = self.encoder(embedding_output,
        ...
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return embedding_output, encoded_layers, pooled_output, all_att
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = UnilmModel(config)
        self.video_trans = nn.Linear(2048, 768)
        # self.trans = nn.Linear(768*2, 768)
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')
        if hasattr(config, 'label_smoothing') and config.label_smoothing:
            self.crit_mask_lm_smoothed = LabelSmoothingLoss(
    ...
    def forward(self, vid_fea, vid_start, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, masked_pos=None, masked_weights=None, num_tokens_a=None, num_tokens_b=None):
        vid_fea = self.video_trans(vid_fea.float())
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, attention_mask, vid_fea, vid_start, output_all_encoded_layers=False) # output after 12 bert layers (encoder)
        ...
        # video_trans = self.video_trans(vid_fea.float())
        # attn2vid = torch.matmul(sequence_output_masked, video_trans.transpose(1,2))
        # attn2vid = F.softmax(attn2vid, dim=-1)
        # output_vid = torch.matmul(attn2vid, video_trans)
        # sequence_output_masked = torch.cat((sequence_output_masked, output_vid), dim=2)
        # sequence_output_masked = self.trans(sequence_output_masked)
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(self, vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask, output_attentions=False):
        vid_fea = self.video_trans(vid_fea)
        if self.search_beam_size > 1:
            return self.beam_search(vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask, output_attentions=output_attentions)
    ...
    def beam_search(self, vid_fea, vid_start, input_ids, token_type_ids, position_ids, attention_mask, output_attentions=False):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        ...
        while next_pos < output_length:
            ...
            curr_attention_mask = attention_mask[:,
                                                 start_pos:next_pos + 1, :next_pos + 1]
            curr_position_ids = position_ids[:, start_pos:next_pos + 1]
            new_embedding, new_encoded_layers, _, all_att = \
                self.bert(x_input_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask, vid_fea, vid_start,
                          output_all_encoded_layers=True, prev_embedding=prev_embedding, prev_encoded_layers=prev_encoded_layers, output_attentions=output_attentions)

            last_hidden = new_encoded_layers[-1][:, -1:, :] #最后一层的hidden bs*seq_len*768 的zuihou 字符的state

            # video_trans = self.video_trans(vid_fea)
            # attn2vid = torch.matmul(last_hidden, video_trans.transpose(1, 2))
            # attn2vid = F.softmax(attn2vid, dim=-1)
            # output_vid = torch.matmul(attn2vid, video_trans)
            # last_hidden = torch.cat((last_hidden, output_vid), dim=2)
            # last_hidden = self.trans(last_hidden)
            ...
            if prev_embedding is None:
                prev_embedding = first_expand(new_embedding[:, :-1, :]) #bert encoder的output bn*seq_len*768 的除最后一个字符mask的output expanddim后(bn*bs)*(seq_len-1)*768
            else:
                prev_embedding = torch.cat(
                    (prev_embedding, new_embedding[:, :-1, :]), dim=1)
            ...
            if prev_encoded_layers is None:
                prev_encoded_layers = [first_expand(
                    x[:, :-1, :]) for x in new_encoded_layers] # encoder中12层transformer的hidden取each的前seq_len-1的值并expanddim为(bn*bs)*(seq_len-1)*768, 组成list
            else:
                prev_encoded_layers = [torch.cat((x[0], x[1][:, :-1, :]), dim=1)
                                       for x in zip(prev_encoded_layers, new_encoded_layers)]
        ...
        return traces, all_attentions, step_ids
