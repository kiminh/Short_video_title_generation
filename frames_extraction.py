...
def extract_frames(video, dst, suffix, strategy, fps, vframes):
    with open(os.devnull, "w") as ffmpeg_log:
        ...
        if strategy == 0:
            ...
            cmd1 = 'ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 %s' % (
            video)
            duration = float(subprocess.check_output(cmd1, shell=True))
...
def run(params):
    ...
    for video_path in tqdm(video_path_list):
        ...
        video_id = video_id.split('video_')[-1]
        ...
        durs.append(dur)
    print(max(durs), min(durs))

if __name__=="__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--video_path", default='../data/video/train_vid', help='the path to load all the videos')
    parser.add_argument("--frame_path", default='../data/frame/train', help='the path to put the extracted frames')
    ...
    parser.add_argument("--fps", type=str, default='3', help='the number of frames you want to extract within 1 second')
    parser.add_argument("--vframes", type=str, default='25', help='the maximun number of frames you want to extract')
