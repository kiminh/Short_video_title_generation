...
def extract_feats(params, model, load_image_fn, C, H, W):
    ...
    for frames_dst in tqdm(frames_path_list):
        ...
        save_path = params['feat_path'] + '/' + video_id
        ...
        if feats.shape[0] != 25:
            pad_mat = np.zeros([25 - feats.shape[0], feats.shape[1]])
            feats = np.concatenate((feats, pad_mat), axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--frame_path", default='../data/frame/train',
                        help='the path to load all the frames')
    parser.add_argument("--feat_path", default='../data/feats/train',
                        help='the path you want to save the features')
    # parser.add_argument("--feat_name", type=str, default='image_resnet101_imagenet_fps_max60.hdf5',
    #                     help='the name of the features file, saved in hdf5 format')
    ...
    # params['feat_dir'] = os.path.join(params['feat_path'],
    #                                   params['feat_name'] + ('' if '.hdf5' in params['feat_name'] else '.hdf5'))
