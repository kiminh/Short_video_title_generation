...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[]):
        ...
        if 'train' in file:
            with open('data/train_vid_2w_include_val.txt', 'r') as ff:
                vid_lists = [line.split('\n')[0] for line in ff]
            self.vid_lists = ['data/train_2w/' + ind for ind in vid_lists]
        if 'test' in file:
            with open('data/test_vid_2w_r0.2.txt', 'r') as ff:
                vid_lists = [line for line in ff]
            self.vid_lists = ['data/test_2w_r0.2/' + ind for ind in vid_lists]
